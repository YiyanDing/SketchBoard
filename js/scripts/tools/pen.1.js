...
class Pen2 extends MovableBlock2{
  constructor(canvas, colorManager, callback) {
    super(canvas, false)
    this.elem = sel('#id-brush2')
    this.strokeStyle = 'black'
    this.enableDraw = false
    this.callback = callback
    this.colorManager2 = colorManager
    this.setupInputs()
  }
  beginDraw(x, y) {
  ...
  drawLine(x, y) {
    ...
    this.ctx.strokeStyle = this.colorManager2.curColor
  ...
  setupInputs() {
    // input range type绑定
    this.addRangeEvt(sel('input[name="line-width"]'), sel('#line-width2'))
    // 鼠标事件
